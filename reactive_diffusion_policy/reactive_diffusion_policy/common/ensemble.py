...
class EnsembleBuffer:
    ...
    #     idx = timestep - self.actions_start_timestep #-8
    #     horizon = action.shape[0]   #25


    #         # 1）整个 chunk 都已经过期：直接丢弃
    #     if idx + horizon <= 0:
    #     # [timestep, timestep + horizon - 1] < actions_start_timestep
    #     # 这批动作对应的时间都已经被 get_action() 消耗完了
    #         return

    # # 2）只有前半部分过期：裁掉过期部分
    #     if idx < 0:
    #     # 过期的步数 = -idx，对应 action 的前 -idx 行
    #         valid_start = -idx
    #         action = action[valid_start:, ...]            # 只保留还没过期的部分
    #         horizon = action.shape[0]
    #         timestep = self.actions_start_timestep        # 新的起点对齐到 buffer 当前起点
    #         idx = 0                                       # 从 0 开始塞

    #     while idx + horizon - 1 >= len(self.actions): #16+1
    #         self.actions.append([])
    #         self.actions_timestep.append([])  #len actions 17



    #     for i in range(idx, idx + horizon):  # i = -9 -8 ..0. 15 # action 0 ...25 
    #         self.actions[i].append(action[i - idx, ...])
    #         self.actions_timestep[i].append(timestep)

    # def add_action(self, action, timestep=None):##simple
    #     """
    #     Add action sequence to the ensemble buffer (simplified version, no timestep alignment).
        
    #     Parameters:
    #     - action: horizon x action_dim array, the action sequence to add
    #     - timestep: ignored (kept for compatibility, but not used)
    #     """
    #     action = np.array(action)  # (horizon, action_dim)
    ...
    #     # 直接把 action 序列的每一步按顺序添加到 buffer 末尾
    #     for i in range(horizon):
    #         self.actions.append([action[i, ...]])  # 每个位置只有一个 action
    #         self.actions_timestep.append([0])  # timestep 不再使用，设为 0 占位
            
    #     print("length",len(self.actions))
    #     print("length",len(self.actions_timestep))
    ...
    #         self.actions_timestep[i].append(timestep)            
    #         ##2.26
    def add_action(self, action, timestep): #orginal
        """
        Add action to the ensemble buffer:
    ...
    def get_action(self):
        ...
        if actions == []:
            return None      # no data
        sorted_actions = sorted(zip(actions_timestep, actions), key=lambda x: x[0])
