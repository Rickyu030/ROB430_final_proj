...
class RealImageTactileDataset(BaseImageDataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        for key in self.rgb_keys:
            ...
            # T,C,H,W
            # resize to shape_meta if mismatch (e.g. zarr from rosbag has different resolution)
            target_shape = self.shape_meta['obs'][key]['shape']  # [C, H, W]
            _, h, w = target_shape[0], target_shape[1], target_shape[2]
            if obs_dict[key].shape[2] != h or obs_dict[key].shape[3] != w:
                # (T, C, H, W) -> per-frame resize to (h, w)
                resized = np.stack([
                    np.moveaxis(
                        cv2.resize(
                            np.moveaxis(obs_dict[key][t], 0, -1),
                            (w, h),
        ...
        for key in self.extended_rgb_keys:
            ...
            # resize to extended_obs shape_meta if mismatch
            target_shape = self.shape_meta['extended_obs'][key]['shape']  # [C, H, W]
            _, h, w = target_shape[0], target_shape[1], target_shape[2]
            if extended_obs_dict[key].shape[2] != h or extended_obs_dict[key].shape[3] != w:
                arr = extended_obs_dict[key]
                resized = np.stack([
                    np.moveaxis(
                        cv2.resize(
                            np.moveaxis(arr[t], 0, -1),
                            (w, h),
                            interpolation=cv2.INTER_LINEAR,
                        ),
                        -1,
