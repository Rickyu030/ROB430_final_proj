...
class CustomRealRunner:
    ...
    def __init__(self,
                 ...
                 action_update_interval: int = 16,  # Single update interval
    ...
    def action_command_thread(self, policy: DiffusionUnetImagePolicy, stop_event):
        ...
        while not stop_event.is_set():
            ...
            if self.use_latent_action_with_rnn_decoder:
                ...
                step_action = step_action[-1]  # Take last step
            else:
...
#                         obs = {
#     # 图像观测 (RGB) - THWC 格式
#     'camera_f': np.ndarray,          # shape: (2, H, W, 3)
#                                        # 例如: (2, 240, 320, 3)
#                                        # dtype: uint8 或 float32
#                                        # 值范围: [0, 255] (如果未归一化)
#                                        # 格式: THWC (时间, 高度, 宽度, 通道)
    
#     'camera_r': np.ndarray,           # shape: (2, H, W, 3)
#                                        # 例如: (2, 240, 320, 3)
#                                        # dtype: uint8 或 float32
#                                        # 格式: THWC
    #  T(horizon) H W 3
#     # 低维观测
#     'Marker_Tracking_Right_DXDY': np.ndarray,  # shape: (2, feature_dim)
#                                                 # 例如: (2, 126)
#                                                 # dtype: float32
    
#     # 关节状态观测
#     'right_arm/joint_states_single': np.ndarray,  # shape: (2, joint_dim)
#                                                     # 例如: (2, 7)  # 7个关节
#                                                     # dtype: float32
#                                                     # 值: 关节位置 (从 JointState.position 提取)
    
#     # 时间戳
#     'timestamp': np.ndarray            # shape: (2,)
#                                        # 例如: (2,)
#                                        # dtype: float64
#                                        # 值: Unix 时间戳（秒）
# }
